...
/* @WENCHUNYANG
 * 根据后面assoc_delete的使用方法，此处不应该是找到上一个位置。
 * 而是找到这个key的位置，然后利用它是二级指针的方式，直接用它的next来替换掉它。
 * 类似于单链表中的删除
 */
...
/*
 * 使用链式的方法来解决冲突，每次都将新插入的元素放到这个链表的卡头部分。
 */
...
/*
 * 维护线程在将旧的哈希表数据复制到新的表时，不是完全一次性全部复制。
 * 这样可能会造成程序暂停很长时间。环境变量控制这个过程。
 * （默认竟然是1。。）
 */
